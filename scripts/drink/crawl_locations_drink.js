// crawl_bars_enhanced_variants_with_geo_wide.js
...
const CATEGORY_SYNONYMS = {
  술집: [
    "술집", "주점", "포차", "호프", "펍", "바",
    "칵테일바", "위스키바", "하이볼바", "와인바", "사케바", "탭룸",
    "이자카야", "선술집", "술공방", "칵테일", "하이볼", "사케", "맥주집",
  ],
};
...
// ===== 상한/콤보 제어(보수적 스타트값) =====
const DISPLAY = 30;
const MAX_PAGES_PER_QUERY = 4;        // 시동 안정
const MAX_QUERIES_PER_CATEGORY = 180; // 필요시 240/300/360으로 ↑
const MAX_ITEMS_PER_CATEGORY   = 400; // 필요시 600/800으로 ↑
...
// ===== 술집 키워드 태깅: "바", "이자카야"만 =====
const BAR_POS = [
  /\bbar\b/i, /바(?!로|지)/, /칵테일\s*바/, /와인\s*바/, /위스키\s*바/, /하이볼\s*바/, /사케\s*바/,
  /칵테일\b/i, /와인\b/i, /위스키\b/i, /하이볼\b/i, /사케\b/i, /탭룸/, /펍/,
];
const IZAKAYA_POS = [/이자카야/, /居酒屋/, /사케\s*바?/];

function inferBarKeywords(textRaw) {
  const t = normalizeText(textRaw);
  const out = [];
  if (matchAny(BAR_POS, t)) out.push("바");
  if (matchAny(IZAKAYA_POS, t)) out.push("이자카야");
  // 요구사항: 둘 다 매치 안 되면 비워둠
  return Array.from(new Set(out)).filter(k => ALLOWED_KEYWORDS.includes(k));
}
...
async function fetchBlogSnippetsStrong({ name, region, category }) {
  ...
  const variants = [
    ...
    `${name} ${region} 칵테일 와인 위스키 하이볼 ${category||""}`,
    `${name} ${region} 이자카야 사케 안주 ${category||""}`,
...
async function upsertLocation(item, catLabel) {
  ...
  if (!extraText || extraText.length < 50) {
    const webFallback = await fetchWebSnippets(
      `${name} ${REGION_CANON} ${catLabel||""} 후기 리뷰 칵테일 와인 위스키 하이볼 이자카야 사케 분위기 조용 웨이팅`
    );
  ...
  const keywordTags = inferBarKeywords(baseTextRaw); // "바", "이자카야"만, 아니면 빈 배열
  ...
  const updatePayload = {
    ...
    keywords: { set: mergedKeywords }, // "바"/"이자카야"만, 없으면 빈 배열
